...
int SidewaysTracker_port, float SidewaysTracker_diameter, float SidewaysTracker_center_distance) :
  ...
  R_ForwardTracker(ForwardTracker_port,true),
...
void Drive::drive_distance(float distance, float heading, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout, float drive_kp, float drive_ki, float drive_kd, float drive_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti){
  ...
  while(drivePID.is_settled() == false){
    ...
    printf("heading error %f average output: %f \n",heading_error,average_position);
...
void Drive::drive_to_pose(float X_position, float Y_position, float angle, float lead, float setback, float drive_min_voltage, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout, float drive_kp, float drive_ki, float drive_kd, float drive_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti){
  ...
  while(!drivePID.is_settled()){
    ...
    printf("Carrot X: %f carrot Y: %f\n",get_X_position(),carrot_Y);
    float drive_output = drivePID.compute(drive_error);
...
void Drive::drive_to_curve(float X_position, float Y_position,float angle,float start_angle,float start_error){
  drive_to_curve(X_position, Y_position,angle,start_angle,start_error,drive_min_voltage, drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_to_curve(float X_position, float Y_position,float angle,float start_angle,float start_error, float drive_min_voltage, float drive_max_voltage, float heading_max_voltage){
  drive_to_curve(X_position, Y_position,angle,start_angle,start_error,drive_min_voltage, drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_to_curve(float X_position, float Y_position,float angle,float start_angle,float start_error, float drive_min_voltage, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout, float drive_kp, float drive_ki, float drive_kd, float drive_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti){
  PID drivePID(hypot(X_position-get_X_position(),Y_position-get_Y_position()), drive_kp, drive_ki, drive_kd, drive_starti, drive_settle_error, drive_settle_time, drive_timeout);
  float start_angle_deg = to_deg(atan2(X_position-get_X_position(),Y_position-get_Y_position()));
  PID headingPID(start_angle_deg-get_absolute_heading(), heading_kp, heading_ki, heading_kd, heading_starti);
  bool line_settled = false;
  bool prev_line_settled = is_line_settled(X_position, Y_position, start_angle_deg, get_X_position(), get_Y_position());
  float start_distance = hypot(X_position-get_X_position(),Y_position-get_Y_position());
  while(!drivePID.is_settled()){
    line_settled = is_line_settled(X_position, Y_position, start_angle_deg, get_X_position(), get_Y_position());
    if(line_settled && !prev_line_settled){ break; }
    prev_line_settled = line_settled;
    ...
    if ((start_distance * start_error) >= drive_error){
      heading_error = reduce_negative_180_to_180(angle-get_absolute_heading()) + reduce_negative_180_to_180(to_deg(atan2(X_position-get_X_position(),Y_position-get_Y_position()))-get_absolute_heading());
    } else {
      heading_error = 0;//reduce_negative_180_to_180(start_angle-get_absolute_heading());
    }
    float drive_output = drivePID.compute(drive_error);
